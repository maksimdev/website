...
export default function({ requisites, bill, saveBill, setData }) {
  ...
  const saveBillRequest = (FN, FD, FDP) => {
    setError(false);
    setLoading(true);
    Api.saveBill(FN, FD, FDP)
      .then(data => {
        setError(false);
        setLoading(false);
  ...
  const getFormattedBill = ({ document: { receipt: { userInn, dateTime, totalSum, items } }}) => {
    ...
    return (
      <Card className={classes.root} variant="outlined">
        <CardHeader
          action={
            <Button disabled={loading} onClick={() => saveBillRequest(requisites.fn, requisites.i, requisites.fp)} color="primary">
